...
function mostrarColores() {
    ...
    mostrarFeedback("paleta de color generado", 3000);
}

function copiarColor(color) {
    navigator.clipboard.writeText(color).then(() => {
        mostrarFeedback("Color copiado", 3000);
    });
}
...
function seleccionarColor(hex, hsl, boton) {
    ...
    if (index === -1) {
        ...
        boton.textContent = '✓';
        mostrarFeedback("Color seleccionado", 2000);
    } else {
        coloresSeleccionados.splice(index, 1);
        boton.style.backgroundColor = '';
        boton.textContent = '★';
        mostrarFeedback("Color deseleccionado", 2000);
    }
}

function guardarPaleta() {
    if (coloresSeleccionados.length === 0) {
        mostrarFeedback("Selecciona colores primero", 3000);
        return;
    }
    ...
    mostrarFeedback("Colores agregados a favoritos", 3000);
}
...
function verFavoritos() {
    ...
    if (favoritosColores.length === 0) {
        mostrarFeedback("No hay colores en favoritos", 3000);
        return;
    }
...
function eliminarColorFavorito(index) {
    ...
    localStorage.setItem('favoritosColores', JSON.stringify(favoritosColores));
    verFavoritos();
    mostrarFeedback("Color eliminado", 3000);
}
...
function limpiarFavoritos() {
    ...
    gridColores.innerHTML = '';
    mostrarFeedback("Favoritos limpiados", 3000);
}

function mostrarFeedback(mensaje, tiempo) {
...
selectorFormato.addEventListener('change', function() {
    ...
    mostrarFeedback(`Cambio a formato ${formatoSeleccionado}`, 3000);
});

selectorTamaño.addEventListener('change', function() {
    const tamañoSeleccionado = selectorTamaño.value;
    mostrarFeedback(`Cambio a tamaño ${tamañoSeleccionado}`, 3000);
});
...
selectorArmonia.addEventListener('change', function() {
    ...
    mostrarFeedback(`Cambio a armonía ${armoniaSeleccionada}`, 3000);
});